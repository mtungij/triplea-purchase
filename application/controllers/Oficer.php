...
public function create_credit_report(){
  return $this->create_report('credit');
}

public function create_collection_report(){
...
public function create_report($report_type = 'credit'){
  ...
  $is_collection_report = ($report_type === 'collection');
  $allowed_positions = $is_collection_report
    ? array('COLLECTION OFFICER', 'COLLECTION OFICER')
    : array('CREDIT OFFICER', 'CREDIT OFICER');

  if (!in_array($position_name, $allowed_positions, true)) {
        return redirect('oficer/index');
    }

  $report_title = $is_collection_report ? 'Collection Officer Report' : 'Credit Officer Report';
  $form_action = $is_collection_report ? 'oficer/create_collection_report' : 'oficer/create_credit_report';
  $report_heading = $is_collection_report ? 'Create Collection Report' : 'Create Credit Report';
  $report_subheading = $is_collection_report ? 'Collection Officer Report' : 'Credit Officer Report';
  $success_message = $is_collection_report ? 'Collection report submitted successfully' : 'Report submitted successfully';
  $report_view = $is_collection_report ? 'oficer/create_collection_report' : 'oficer/create_report';

    if ($this->input->post()) {
    if ($is_collection_report) {
      $total_amount_due_today = (float) $this->input->post('total_amount_due_today');
      $total_amount_collected = (float) $this->input->post('total_amount_collected');
      $collection_efficiency = (float) $this->input->post('collection_efficiency');

      $clients_who_paid = (int) $this->input->post('clients_who_paid');
      $clients_who_did_not_pay = (int) $this->input->post('clients_who_did_not_pay');
      $partial_payments = (int) $this->input->post('partial_payments');

      $calls_made = (int) $this->input->post('calls_made');
      $sms_reminders_sent = (int) $this->input->post('sms_reminders_sent');
      $field_visits_conducted = (int) $this->input->post('field_visits_conducted');

      $promises_to_pay = (int) $this->input->post('promises_to_pay');
      $payments_secured_after_follow_up = (int) $this->input->post('payments_secured_after_follow_up');
      $clients_unreachable = (int) $this->input->post('clients_unreachable');

      $clients_in_arrears_1_30_days = (int) $this->input->post('clients_in_arrears_1_30_days');
      $clients_in_arrears_over_30_days = (int) $this->input->post('clients_in_arrears_over_30_days');
      $par_over_30_days_tzs = (float) $this->input->post('par_over_30_days_tzs');
      $high_risk_clients_identified = trim((string) $this->input->post('high_risk_clients_identified'));

      $cases_escalated_to_recovery_officer = (int) $this->input->post('cases_escalated_to_recovery_officer');
      $guarantors_contacted = (int) $this->input->post('guarantors_contacted');
      $collateral_follow_up_initiated = (int) $this->input->post('collateral_follow_up_initiated');

      $potential_clients_identified = (int) $this->input->post('potential_clients_identified');
      $source_of_leads = $this->input->post('source_of_leads');
      $loan_inquiries_received = (int) $this->input->post('loan_inquiries_received');
      $clients_referred_to_marketing_credit = (int) $this->input->post('clients_referred_to_marketing_credit');
      $client_quality = trim((string) $this->input->post('client_quality'));

      $client_complaints_received = (int) $this->input->post('client_complaints_received');
      $complaints_resolved = (int) $this->input->post('complaints_resolved');
      $client_feedback = trim((string) $this->input->post('client_feedback'));

      $cash_collected = (float) $this->input->post('cash_collected');
      $mobile_money_collected = (float) $this->input->post('mobile_money_collected');
      $bank_deposits_made = (float) $this->input->post('bank_deposits_made');
      $reconciliation_completed = trim((string) $this->input->post('reconciliation_completed'));
      $collection_challenges_faced = trim((string) $this->input->post('collection_challenges_faced'));
      $collection_action_plan_next_day = trim((string) $this->input->post('collection_action_plan_next_day'));

      if (!is_array($source_of_leads) || empty($source_of_leads)) {
        $source_of_leads = array('None');
      }

      if ($client_quality === '') {
        $client_quality = 'Not selected';
      }

      if ($reconciliation_completed === '') {
        $reconciliation_completed = 'Not selected';
      }

      $report_body = "1. COLLECTION PERFORMANCE\n";
      $report_body .= "1.1 Daily Collection Summary\n";
      $report_body .= "a) Total amount due today (TZS): " . number_format($total_amount_due_today, 2, '.', '') . "\n";
      $report_body .= "b) Total amount collected (TZS): " . number_format($total_amount_collected, 2, '.', '') . "\n";
      $report_body .= "c) Collection efficiency (%): " . number_format($collection_efficiency, 2, '.', '') . "\n\n";
      $report_body .= "1.2 Client Payment Status\n";
      $report_body .= "a) Number of clients who paid: " . $clients_who_paid . "\n";
      $report_body .= "b) Number of clients who did not pay: " . $clients_who_did_not_pay . "\n";
      $report_body .= "c) Partial payments: " . $partial_payments . "\n\n";
      $report_body .= "2. FOLLOW-UP ACTIVITIES\n";
      $report_body .= "a) Calls made: " . $calls_made . "\n";
      $report_body .= "b) SMS reminders sent: " . $sms_reminders_sent . "\n";
      $report_body .= "c) Field visits conducted: " . $field_visits_conducted . "\n\n";
      $report_body .= "Follow-Up Outcomes\n";
      $report_body .= "a) Promises to pay: " . $promises_to_pay . "\n";
      $report_body .= "b) Payments secured after follow-up: " . $payments_secured_after_follow_up . "\n";
      $report_body .= "c) Clients unreachable: " . $clients_unreachable . "\n\n";
      $report_body .= "3. ARREARS & PAR MONITORING\n";
      $report_body .= "- Clients in arrears (1-30 days): " . $clients_in_arrears_1_30_days . "\n";
      $report_body .= "- Clients in arrears (>30 days): " . $clients_in_arrears_over_30_days . "\n";
      $report_body .= "- PAR >30 days (TZS): " . number_format($par_over_30_days_tzs, 2, '.', '') . "\n";
      $report_body .= "High-Risk Clients Identified: " . $high_risk_clients_identified . "\n";
      $report_body .= "\n4. RECOVERY SUPPORT ACTIVITIES\n";
      $report_body .= "a) Cases escalated to Recovery Officer: " . $cases_escalated_to_recovery_officer . "\n";
      $report_body .= "b) Guarantors contacted: " . $guarantors_contacted . "\n";
      $report_body .= "c) Collateral follow-up initiated: " . $collateral_follow_up_initiated . "\n";
      $report_body .= "\n5. CLIENT ACQUISITION\n";
      $report_body .= "5.1 Leads Generated\n";
      $report_body .= "a) Number of potential clients identified: " . $potential_clients_identified . "\n";
      $report_body .= "b) Source: " . implode(', ', $source_of_leads) . "\n";
      $report_body .= "5.2 Loan Opportunities\n";
      $report_body .= "a) Loan inquiries received: " . $loan_inquiries_received . "\n";
      $report_body .= "b) Clients referred to Marketing/Credit: " . $clients_referred_to_marketing_credit . "\n";
      $report_body .= "5.3 Client Quality\n";
      $report_body .= "Selected quality: " . $client_quality . "\n";
      $report_body .= "\n6. CUSTOMER RELATIONSHIP MANAGEMENT\n";
      $report_body .= "1. Client complaints received: " . $client_complaints_received . "\n";
      $report_body .= "2. Complaints resolved: " . $complaints_resolved . "\n";
      $report_body .= "Client Feedback: " . $client_feedback . "\n";
      $report_body .= "\n7. CASH & RECONCILIATION\n";
      $report_body .= "1. Cash collected: " . number_format($cash_collected, 2, '.', '') . "\n";
      $report_body .= "2. Mobile money collected: " . number_format($mobile_money_collected, 2, '.', '') . "\n";
      $report_body .= "3. Bank deposits made: " . number_format($bank_deposits_made, 2, '.', '') . "\n";
      $report_body .= "4. Reconciliation completed: " . $reconciliation_completed . "\n";
      $report_body .= "\n8. CHALLENGES FACED\n";
      $report_body .= $collection_challenges_faced . "\n";
      $report_body .= "\n9. ACTION PLAN (NEXT DAY)\n";
      $report_body .= $collection_action_plan_next_day . "\n";
      ...
      $this->queries->insert_collection_officer_report($data);
      $this->session->set_flashdata('massage', $success_message);
      return redirect($form_action);
    }

    $loan_applications_received = (int) $this->input->post('loan_applications_received');
    $applications_reviewed = (int) $this->input->post('applications_reviewed');
    $applications_approved = (int) $this->input->post('applications_approved');
    $applications_rejected = (int) $this->input->post('applications_rejected');
    $applications_pending = (int) $this->input->post('applications_pending');
      $total_amount_applied = (float) $this->input->post('total_amount_applied');
      $total_amount_approved = (float) $this->input->post('total_amount_approved');
    $clients_assessed = (int) $this->input->post('clients_assessed');
    $average_dbr = (float) $this->input->post('average_dbr');
    $high_risk_applications_identified = (int) $this->input->post('high_risk_applications_identified');
    $over_indebted_clients_detected = (int) $this->input->post('over_indebted_clients_detected');
    $business_visits_conducted = (int) $this->input->post('business_visits_conducted');
    $residence_visits_conducted = (int) $this->input->post('residence_visits_conducted');
    $clients_verified_successfully = (int) $this->input->post('clients_verified_successfully');
    $cases_with_discrepancies = (int) $this->input->post('cases_with_discrepancies');
    $new_clients_sourced = (int) $this->input->post('new_clients_sourced');
    $leads_generated = (int) $this->input->post('leads_generated');
    $loan_application_from_own_sourcing = (int) $this->input->post('loan_application_from_own_sourcing');
    $client_quality_assessment = $this->input->post('client_quality_assessment');
    $kyc_completed_cases = (int) $this->input->post('kyc_completed_cases');
    $files_fully_documented = (int) $this->input->post('files_fully_documented');
    $incomplete_documentation_cases = (int) $this->input->post('incomplete_documentation_cases');
    $number_of_active_clients = (int) $this->input->post('number_of_active_clients');
    $clients_with_on_time_payments = (int) $this->input->post('clients_with_on_time_payments');
    $clients_in_arrears = (int) $this->input->post('clients_in_arrears');
    $par_1_30_days = (float) $this->input->post('par_1_30_days');
    $par_over_30_days = (float) $this->input->post('par_over_30_days');
    $cases_submitted_to_credit_committee = $this->input->post('cases_submitted_to_credit_committee');
    $follow_ups_with_collections = $this->input->post('follow_ups_with_collections');
    $follow_ups_with_recovery = $this->input->post('follow_ups_with_recovery');
    $weekly_financial_visibility = $this->input->post('weekly_financial_visibility');
    $monthly_financial_statement = $this->input->post('monthly_financial_statement');
    $sales_loan_budget_tracking = $this->input->post('sales_loan_budget_tracking');
    $challenges_faced = $this->input->post('challenges_faced');
    $action_plan_next_day = $this->input->post('action_plan_next_day');

    if (!$client_quality_assessment) {
      $client_quality_assessment = 'Not selected';
    }
    if (!$weekly_financial_visibility) {
      $weekly_financial_visibility = 'Not selected';
    }
    if (!$monthly_financial_statement) {
      $monthly_financial_statement = 'Not selected';
    }
    if (!$sales_loan_budget_tracking) {
      $sales_loan_budget_tracking = 'Not selected';
    }

    $report_body = "1.LOAN PROCESSING ACTIVITIES\n";
    $report_body .= "1.1 Applications Management\n";
    $report_body .= "a) Loan applications received: " . $loan_applications_received . "\n";
    $report_body .= "b) Applications reviewed: " . $applications_reviewed . "\n";
    $report_body .= "c) Applications approved: " . $applications_approved . "\n";
    $report_body .= "d) Applications rejected: " . $applications_rejected . "\n";
    $report_body .= "e) Applications pending: " . $applications_pending . "\n\n";
      $report_body .= "1.2 Value of Loans\n";
      $report_body .= "a) Total Amount Applied (TZS): " . number_format($total_amount_applied, 2, '.', '') . "\n";
      $report_body .= "b) Total Amount Approved (TZS): " . number_format($total_amount_approved, 2, '.', '') . "\n\n";
      $report_body .= "2. CREDIT ANALYSIS & RISK ASSESSMENT\n";
      $report_body .= "a) Number of clients assessed: " . $clients_assessed . "\n";
      $report_body .= "b) Average DBR(%): " . number_format($average_dbr, 2, '.', '') . "\n";
      $report_body .= "c) High-risk applications identified: " . $high_risk_applications_identified . "\n";
      $report_body .= "d) Over-indebted clients detected: " . $over_indebted_clients_detected . "\n\n";
      $report_body .= "Risk Observations:\n\n";
      $report_body .= "3. FIELD VERIFICATION ACTIVITIES\n";
      $report_body .= "a) Business visits conducted: " . $business_visits_conducted . "\n";
      $report_body .= "b) Residence visits conducted: " . $residence_visits_conducted . "\n";
      $report_body .= "c) Clients verified successfully: " . $clients_verified_successfully . "\n";
      $report_body .= "d) Cases with discrepancies: " . $cases_with_discrepancies . "\n\n";
      $report_body .= "4. CLIENT ACQUISITION\n";
      $report_body .= "a) New clients sourced: " . $new_clients_sourced . "\n";
      $report_body .= "b) Leads generated: " . $leads_generated . "\n";
      $report_body .= "c) Loan applications from own sourcing: " . $loan_application_from_own_sourcing . "\n\n";
      $report_body .= "Client Quality Assessment:\n";
      $report_body .= "Selected assessment: " . $client_quality_assessment . "\n\n";
      $report_body .= "5. PORTFOLIO MONITORING\n";
      $report_body .= "a) Number of active clients: " . $number_of_active_clients . "\n";
      $report_body .= "b) Clients with on-time payments: " . $clients_with_on_time_payments . "\n";
      $report_body .= "c) Clients in arrears: " . $clients_in_arrears . "\n\n";
      $report_body .= "PAR STATUS\n";
      $report_body .= "a) PAR 1-30 days (TZS): " . number_format($par_1_30_days, 2, '.', '') . "\n";
      $report_body .= "b) PAR >30 days (TZS): " . number_format($par_over_30_days, 2, '.', '') . "\n\n";
      $report_body .= "6. COMPLIANCE & DOCUMENTATION\n";
      $report_body .= "a) KYC completed cases: " . $kyc_completed_cases . "\n";
      $report_body .= "b) Files fully documented: " . $files_fully_documented . "\n";
      $report_body .= "c) Incomplete documentation cases: " . $incomplete_documentation_cases . "\n\n";
      $report_body .= "7. COORDINATION ACTIVITIES\n";
      $report_body .= "a) Cases submitted to Credit Committee: " . $cases_submitted_to_credit_committee . "\n";
      $report_body .= "b) Follow-ups with Collections: " . $follow_ups_with_collections . "\n";
      $report_body .= "c) Follow-ups with Recovery: " . $follow_ups_with_recovery . "\n\n";
      $report_body .= "8. FINANCIAL REPORTING TASKS\n";
      $report_body .= "a) Weekly financial visibility updated: " . $weekly_financial_visibility . "\n";
      $report_body .= "b) Monthly financial statement progress: " . $monthly_financial_statement . "\n";
      $report_body .= "c) Sales/loan budget tracking updated: " . $sales_loan_budget_tracking . "\n\n";
      $report_body .= "9. CHALLENGES FACED\n";
      $report_body .= $challenges_faced . "\n\n";
      $report_body .= "10. ACTION PLAN (NEXT DAY)\n";
      $report_body .= $action_plan_next_day;
      ...
      'report_body'  => $report_body,
        ...
        $this->queries->insert_credit_officer_report($data);
          $this->session->set_flashdata('massage', $success_message);
          return redirect($form_action);
    }
    ...
    $view_data = array(
      ...
      'report_subheading' => $report_subheading,
      'form_action' => $form_action,
    );
    $this->load->view($report_view, $view_data);
}

  public function create_insurance_report(){
    $this->load->model('queries');
    $empl_id = $this->session->userdata('empl_id');
    $manager_data = $this->queries->get_manager_data($empl_id);
    $comp_id = $manager_data->comp_id;
    ...
    if (!in_array($position_name, array('INSURANCE OFFICER', 'INSURANCE OFICER'), true)) {
      return redirect('oficer/index');
    }

    if ($this->input->post()) {
      $new_insurance_clients_identified = (int) $this->input->post('new_insurance_clients_identified');
      $new_insurance_policies_issued = (int) $this->input->post('new_insurance_policies_issued');
      $loan_clients_converted_to_insurance = (int) $this->input->post('loan_clients_converted_to_insurance');
      $leads_generated = (int) $this->input->post('leads_generated');
      $source_of_clients = $this->input->post('source_of_clients');

      $total_insurance_premium_generated = (float) $this->input->post('total_insurance_premium_generated');
      $loan_linked_insurance_contribution = (float) $this->input->post('loan_linked_insurance_contribution');
      $initial_client_contribution_collected = (float) $this->input->post('initial_client_contribution_collected');

      $field_visits_conducted = (int) $this->input->post('field_visits_conducted');
      $clients_visited = (int) $this->input->post('clients_visited');
      $business_sme_clients_engaged = (int) $this->input->post('business_sme_clients_engaged');
      $insurance_awareness_sessions_conducted = (int) $this->input->post('insurance_awareness_sessions_conducted');

      $insurance_applications_received = (int) $this->input->post('insurance_applications_received');
      $policies_processed_registered = (int) $this->input->post('policies_processed_registered');
      $pending_applications = (int) $this->input->post('pending_applications');
      $completed_documentation_rate = (float) $this->input->post('completed_documentation_rate');

      $claims_received = (int) $this->input->post('claims_received');
      $claims_processed = (int) $this->input->post('claims_processed');
      $claims_pending_verification = (int) $this->input->post('claims_pending_verification');
      $claims_resolved_today = (int) $this->input->post('claims_resolved_today');

      $loan_clients_covered_today = (int) $this->input->post('loan_clients_covered_today');
      $uninsured_loan_clients_identified = (int) $this->input->post('uninsured_loan_clients_identified');
      $follow_up_actions_taken = trim((string) $this->input->post('follow_up_actions_taken'));

      $fully_compliant_cases = (int) $this->input->post('fully_compliant_cases');
      $documentation_errors = (int) $this->input->post('documentation_errors');
      $fraud_risk_alerts = (int) $this->input->post('fraud_risk_alerts');

      $clients_educated_on_insurance = (int) $this->input->post('clients_educated_on_insurance');
      $product_explanations_conducted = (int) $this->input->post('product_explanations_conducted');
      $client_inquiries_handled = (int) $this->input->post('client_inquiries_handled');
      $insurance_challenges_faced = trim((string) $this->input->post('insurance_challenges_faced'));
      $insurance_action_plan_next_day = trim((string) $this->input->post('insurance_action_plan_next_day'));

      if (!is_array($source_of_clients) || empty($source_of_clients)) {
        $source_of_clients = array('None');
      }

      $report_body = "1. CLIENT ACQUISITION (SALES PERFORMANCE)\n";
      $report_body .= "a) New insurance clients identified: " . $new_insurance_clients_identified . "\n";
      $report_body .= "b) New insurance policies issued: " . $new_insurance_policies_issued . "\n";
      $report_body .= "c) Loan clients converted to insurance: " . $loan_clients_converted_to_insurance . "\n";
      $report_body .= "d) Leads generated: " . $leads_generated . "\n";
      $report_body .= "Source of Clients: " . implode(', ', $source_of_clients) . "\n\n";

      $report_body .= "2. SALES VALUE PERFORMANCE\n";
      $report_body .= "a) Total insurance premium generated (TZS): " . number_format($total_insurance_premium_generated, 2, '.', '') . "\n";
      $report_body .= "b) Loan-linked insurance contribution (TZS): " . number_format($loan_linked_insurance_contribution, 2, '.', '') . "\n";
      $report_body .= "c) Initial client contribution collected (if applicable): " . number_format($initial_client_contribution_collected, 2, '.', '') . "\n\n";

      $report_body .= "3. FIELD ACTIVITIES\n";
      $report_body .= "a) Field visits conducted: " . $field_visits_conducted . "\n";
      $report_body .= "b) Clients visited: " . $clients_visited . "\n";
      $report_body .= "c) Business/SME clients engaged: " . $business_sme_clients_engaged . "\n";
      $report_body .= "d) Insurance awareness sessions conducted: " . $insurance_awareness_sessions_conducted . "\n\n";

      $report_body .= "4. POLICY ADMINISTRATION\n";
      $report_body .= "a) Insurance applications received: " . $insurance_applications_received . "\n";
      $report_body .= "b) Policies processed and registered: " . $policies_processed_registered . "\n";
      $report_body .= "c) Pending applications: " . $pending_applications . "\n";
      $report_body .= "d) Completed documentation rate: " . number_format($completed_documentation_rate, 2, '.', '') . " %\n\n";

      $report_body .= "5. CLAIMS MANAGEMENT\n";
      $report_body .= "a) Claims received: " . $claims_received . "\n";
      $report_body .= "b) Claims processed: " . $claims_processed . "\n";
      $report_body .= "c) Claims pending verification: " . $claims_pending_verification . "\n";
      $report_body .= "d) Claims resolved today: " . $claims_resolved_today . "\n\n";

      $report_body .= "6. LOAN-INSURANCE LINKAGE\n";
      $report_body .= "a) Loan clients covered by insurance today: " . $loan_clients_covered_today . "\n";
      $report_body .= "b) Uninsured loan clients identified: " . $uninsured_loan_clients_identified . "\n";
      $report_body .= "c) Follow-up actions taken: " . $follow_up_actions_taken . "\n\n";

      $report_body .= "7. COMPLIANCE & RISK CONTROL\n";
      $report_body .= "a) Fully compliant cases: " . $fully_compliant_cases . "\n";
      $report_body .= "b) Documentation errors: " . $documentation_errors . "\n";
      $report_body .= "c) Fraud/risk alerts: " . $fraud_risk_alerts . "\n\n";

      $report_body .= "8. CLIENT COMMUNICATION & EDUCATION\n";
      $report_body .= "a) Clients educated on insurance: " . $clients_educated_on_insurance . "\n";
      $report_body .= "b) Product explanations conducted: " . $product_explanations_conducted . "\n";
      $report_body .= "c) Client inquiries handled: " . $client_inquiries_handled . "\n\n";

      $report_body .= "9. CHALLENGES FACED\n";
      $report_body .= $insurance_challenges_faced . "\n\n";

      $report_body .= "10. ACTION PLAN (NEXT DAY)\n";
      $report_body .= $insurance_action_plan_next_day;

      $data = array(
        'empl_id'      => $empl_id,
        'comp_id'      => $comp_id,
        'blanch_id'    => $blanch_id,
        'report_title' => 'Insurance Officer Report',
        'report_body'  => $report_body,
        'report_date'  => date('Y-m-d'),
      );

      $this->queries->insert_insurance_officer_report($data);
      $this->session->set_flashdata('massage', 'Insurance report submitted successfully');
      return redirect('oficer/create_insurance_report');
    }
    ...
    $this->load->view('oficer/create_insurance_report', $view_data);
  }
