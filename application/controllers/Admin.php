...
echo $this->queries->fetch_loan_active_day($this->input->post('customer_id'));
  ...
  public function officer_daily_reports(){
    $this->load->model('queries');
    $comp_id = $this->session->userdata('comp_id');

    $from_date   = $this->input->get('from_date')   ?: null;
    $to_date     = $this->input->get('to_date')     ?: null;
    $report_type = $this->input->get('report_type') ?: 'all';

    // Sanitise dates — accept only YYYY-MM-DD
    $from_date = ($from_date && preg_match('/^\d{4}-\d{2}-\d{2}$/', $from_date)) ? $from_date : null;
    $to_date   = ($to_date   && preg_match('/^\d{4}-\d{2}-\d{2}$/', $to_date))   ? $to_date   : null;

    $credit_reports     = ($report_type === 'all' || $report_type === 'credit')
                          ? $this->queries->get_credit_officer_reports($comp_id, $from_date, $to_date)     : [];
    $insurance_reports  = ($report_type === 'all' || $report_type === 'insurance')
  ...
  public function officer_daily_report_view($report_id){
    $this->load->model('queries');
    $comp_id = $this->session->userdata('comp_id');
    $report = $this->queries->get_credit_officer_report($report_id, $comp_id);
  ...
  public function officer_daily_report_download($report_id){
    $this->load->model('queries');
    $comp_id = $this->session->userdata('comp_id');
    $compdata = $this->queries->get_companyData($comp_id);
    $report = $this->queries->get_credit_officer_report($report_id, $comp_id);
    ...
    $filename = 'credit_report_' . $creatorSlug . '_' . $reportDate . '.pdf';
    $mpdf->Output($filename, 'D');
  }

  public function insurance_daily_report_view($report_id){
    $this->load->model('queries');
    $comp_id = $this->session->userdata('comp_id');
    $report = $this->queries->get_insurance_officer_report($report_id, $comp_id);
  ...
  public function insurance_daily_report_download($report_id){
    $this->load->model('queries');
    $comp_id = $this->session->userdata('comp_id');
    $compdata = $this->queries->get_companyData($comp_id);
    $report = $this->queries->get_insurance_officer_report($report_id, $comp_id);
    ...
    $reportDate = !empty($report->report_date) ? date('Ymd', strtotime($report->report_date)) : date('Ymd');
    $filename = 'insurance_report_' . $creatorSlug . '_' . $reportDate . '.pdf';
    $mpdf->Output($filename, 'D');
  }

  public function collection_daily_report_view($report_id){
    $this->load->model('queries');
    $comp_id = $this->session->userdata('comp_id');
    $report = $this->queries->get_collection_officer_report($report_id, $comp_id);

    if (!$report) {
      show_404();
    }

    $report->report_title = 'Collection Officer Report';

    $this->load->view('admin/credit_officer_daily_report_view', ['report' => $report]);
  }

  public function collection_daily_report_download($report_id){
    $this->load->model('queries');
    $comp_id = $this->session->userdata('comp_id');
    $compdata = $this->queries->get_companyData($comp_id);
    $report = $this->queries->get_collection_officer_report($report_id, $comp_id);
